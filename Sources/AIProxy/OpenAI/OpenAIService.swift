...
@AIProxyActor public class OpenAIService: Sendable {
    ...
    public func realtimeSession(
        ...
        let connection = try self.makeRealtimeNWConnection(for: request)
    ...
    private func makeRealtimeNWConnection(for request: URLRequest) throws -> NWConnection {
        ...
        // Realtime uses NWConnection (not URLSession), so apply AIProxy pinning only for proxied services.
        if !(self.serviceNetworker is DirectService) {
